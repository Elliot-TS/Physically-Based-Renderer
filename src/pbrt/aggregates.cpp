...
{
  const int ps = 51;
  if (primitives.size() != ps)
    std::cout << "A. primitives.size() = " << primitives.size()
              << std::endl;
  ...
  if (bvhPrimitives.size() != ps)
    std::cout << "A. bvhPrimitives.size() = "
              << bvhPrimitives.size() << std::endl;
  ...
  if (orderedPrims.size() != ps)
    std::cout << "A. orderedPrims.size() = "
              << orderedPrims.size() << std::endl;
  ...
  LinearBVHNode *a = &nodes[0];
  LinearBVHNode *b = &nodes[1];
  int offset = 0;
...
)
{
  ...
  for (auto prim : bvhPrimitives) {
    ...
    orderedPrims.push_back(primitives[prim.primitiveIndex]);
  }

  node->InitLeaf(
      *orderedPrimsOffset, bvhPrimitives.size(), bounds
  );
...
/*
BVHAggregate::BVHAggregate(
    std::vector<Primitive *> primitives, int maxPrimsInNode,
    SplitMethod splitMethod
)
    : maxPrimsInNode(std::min(255, maxPrimsInNode)),
      ...
      splitMethod(splitMethod)
{
  ...
  if (splitMethod == SplitMethod::HLBVH) {
    ERROR(
        "SplitMethod::HLBVH is not implemented.  Choose a
different method " "for generating the BVH"
    );
    // root = buildHLBVH(bhvPrimitives, &totalNodes,
    // orderedPrims);
  }
  else {
    std::atomic<int> orderedPrimsOffset {0};
    root = buildRecursive(
        std::span<BVHPrimitive>(bvhPrimitives), &totalNodes,
...
)
{
  ...
  auto createLeafNode = [&](BVHBuildNode *node) {
    ...
    node->InitLeaf(
        firstPrimOffset, bvhPrimitives.size(), bounds
    );
  };
  ...
  if (bounds.SurfaceArea() == 0 || bvhPrimitives.size() == 1) {
    ...
    // Compute bounds of primitive centroids and choose a split
    // dimension
    ...
    if (centroidBounds.pMax[dim] == centroidBounds.pMin[dim]) {
      ...
      // TODO: Make this parallel if bvhPrimitives.size() >
      // 128*1024
...
) const
{
  ...
  Vector3f invDir(
      1 / ray.direction.x, 1 / ray.direction.y,
      1 / ray.direction.z
  );
  int dirIsNeg[3] = {
  ...
  // Follow ray through BVH nodes to find primitive
  // intersections
