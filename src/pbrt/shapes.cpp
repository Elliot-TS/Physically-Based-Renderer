...
namespace pbrt {
    ...
    Bounds3f Triangle::Bounds() const {
        // Get triangle vertices p0, p1, and p2
    ...
    std::vector<std::unique_ptr<Shape>> Triangle::CreateTriangles(const TriangleMesh *mesh) {
         ...
         Point3f p0, Point3f p1, Point3f p2)
    {
        ...
        if (det < 0 && (tScaled >= 0 || tScaled < tMax * det)) return {};
        else if (det > 0 && (tScaled <= 0 || tScaled > tMax * det)) return {};
        ...
        // Ensure that computed triangle t is conservatively greater than zero TODO
        ...
        (const Ray &ray, Float tMax) const
    {
        ...
        std::optional<TriangleIntersection> ti = IntersectTriangle(ray, tMax, p0, p1, p2);
