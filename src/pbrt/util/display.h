...
namespace pbrt{
    ...
    class DisplayWindow {
        ...
        private:
            ...
            bool initSDL() { SDL_is_initialized = true; return SDL_Init(SDL_INIT_VIDEO); }
            bool initWindow() { 
            ...
            bool initAll() {
                if (!(
                            ASSERT(initSDL(), "Error Initializing SDL: " << SDL_GetError()) &&
                            ASSERT(initWindow(), "Error Creating window: " << SDL_GetError()) &&
                            ASSERT(initRenderer(), "Error Creating renderer: " << SDL_GetError()) &&
                            ASSERT(initTexture(), "Error creating texture: " << SDL_GetError())
                     ))
                {
            ...
            void closeWindow() {
                ...
                if (!isOpen) { closingWindow.unlock(); return; }
