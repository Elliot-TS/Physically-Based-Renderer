...
Point3fi Transform::ApplyInverse(const Point3fi &p) const
{
  ...
  Float xp = (mInv[0][0] * x + mInv[0][1] * y) +
             (mInv[0][2] * z + mInv[0][3]);
  Float yp = (mInv[1][0] * x + mInv[1][1] * y) +
             (mInv[1][2] * z + mInv[1][3]);
  Float zp = (mInv[2][0] * x + mInv[2][1] * y) +
             (mInv[2][2] * z + mInv[2][3]);
  Float wp = (mInv[3][0] * x + mInv[3][1] * y) +
             (mInv[3][2] * z + mInv[3][3]);

  // Compute absolute error for transformed point
  Vector3f pOutError;
  if (p.IsExact()) {
    pOutError.x = gamma(3) * (std::abs(mInv[0][0] * x) +
                              std::abs(mInv[0][1] * y) +
                              std::abs(mInv[0][2] * z));
    pOutError.y = gamma(3) * (std::abs(mInv[1][0] * x) +
                              std::abs(mInv[1][1] * y) +
                              std::abs(mInv[1][2] * z));
    pOutError.z = gamma(3) * (std::abs(mInv[2][0] * x) +
                              std::abs(mInv[2][1] * y) +
                              std::abs(mInv[2][2] * z));
  }
  else {
    ...
    pOutError.x =
        (gamma(3) + 1) * (std::abs(mInv[0][0]) * pInError.x +
    ...
    pOutError.y =
        (gamma(3) + 1) * (std::abs(mInv[1][0]) * pInError.x +
    ...
    pOutError.z =
        (gamma(3) + 1) * (std::abs(mInv[2][0]) * pInError.x +
      ...
      actuallyAnimated(startTransform != endTransform)
{
  ...
  if (hasRotation) {
    ...
    c1[0] = DerivativeTerm(
        -t0x + t1x,
        (-1 + q0y * q0y + q0z * q0z + qperpy * qperpy +
         qperpz * qperpz) *
                s000 +
            ...
            q0w * q0z * s110 + qperpx * qperpy * s110 -
            qperpw * qperpz * s110 + q0w * q0y * s120 +
            qperpw * qperpy * s120 + qperpx * qperpz * s120 +
            q0x * (-(q0y * s010) - q0z * s020 + q0y * s110 +
                   q0z * s120),
        (-1 + q0y * q0y + q0z * q0z + qperpy * qperpy +
         qperpz * qperpz) *
                s001 +
            ...
            q0w * q0z * s111 + qperpx * qperpy * s111 -
            qperpw * qperpz * s111 + q0w * q0y * s121 +
            qperpw * qperpy * s121 + qperpx * qperpz * s121 +
            q0x * (-(q0y * s011) - q0z * s021 + q0y * s111 +
                   q0z * s121),
        (-1 + q0y * q0y + q0z * q0z + qperpy * qperpy +
         qperpz * qperpz) *
                s002 +
            ...
            q0w * q0z * s112 + qperpx * qperpy * s112 -
            qperpw * qperpz * s112 + q0w * q0y * s122 +
            qperpw * qperpy * s122 + qperpx * qperpz * s122 +
            q0x * (-(q0y * s012) - q0z * s022 + q0y * s112 +
                   q0z * s122)
    );
    ...
    c2[0] = DerivativeTerm(
        ...
        -(qperpy * qperpy * s000) - qperpz * qperpz * s000 +
            ...
            qperpw * qperpy * s020 + qperpx * qperpz * s020 +
            q0y * q0y * (s000 - s100) +
            q0z * q0z * (s000 - s100) + qperpy * qperpy * s100 +
        ...
        -(qperpy * qperpy * s001) - qperpz * qperpz * s001 +
            ...
            qperpw * qperpy * s021 + qperpx * qperpz * s021 +
            q0y * q0y * (s001 - s101) +
            q0z * q0z * (s001 - s101) + qperpy * qperpy * s101 +
        ...
        -(qperpy * qperpy * s002) - qperpz * qperpz * s002 +
            ...
            qperpw * qperpy * s022 + qperpx * qperpz * s022 +
            q0y * q0y * (s002 - s102) +
            q0z * q0z * (s002 - s102) + qperpy * qperpy * s102 +
    ...
    c3[0] = DerivativeTerm(
        ...
        -2 *
            (q0x * qperpy * s010 - q0w * qperpz * s010 +
             ...
             q0y * (-2 * qperpy * s000 + qperpx * s010 +
                    qperpw * s020 + 2 * qperpy * s100 -
                    qperpx * s110 - qperpw * s120) +
             q0z * (-2 * qperpz * s000 - qperpw * s010 +
                    qperpx * s020 + 2 * qperpz * s100 +
        ...
        -2 *
            (q0x * qperpy * s011 - q0w * qperpz * s011 +
             ...
             q0y * (-2 * qperpy * s001 + qperpx * s011 +
                    qperpw * s021 + 2 * qperpy * s101 -
                    qperpx * s111 - qperpw * s121) +
             q0z * (-2 * qperpz * s001 - qperpw * s011 +
                    qperpx * s021 + 2 * qperpz * s101 +
        ...
        -2 *
            (q0x * qperpy * s012 - q0w * qperpz * s012 +
             ...
             q0y * (-2 * qperpy * s002 + qperpx * s012 +
                    qperpw * s022 + 2 * qperpy * s102 -
                    qperpx * s112 - qperpw * s122) +
             q0z * (-2 * qperpz * s002 - qperpw * s012 +
                    qperpx * s022 + 2 * qperpz * s102 +
    ...
    c5[0] = DerivativeTerm(
        ...
        2 *
            (qperpy * qperpy * s000 + qperpz * qperpz * s000 -
             ...
             q0y * (q0x * (s010 - s110) + q0w * (s020 - s120)) +
             qperpw * qperpy * s120 + qperpx * qperpz * s120 +
             q0z * (-(q0w * s010) + q0x * s020 + q0w * s110 -
                    q0x * s120)) *
            theta,
        2 *
            (qperpy * qperpy * s001 + qperpz * qperpz * s001 -
             ...
             q0y * (q0x * (s011 - s111) + q0w * (s021 - s121)) +
             qperpw * qperpy * s121 + qperpx * qperpz * s121 +
             q0z * (-(q0w * s011) + q0x * s021 + q0w * s111 -
                    q0x * s121)) *
            theta,
        2 *
            (qperpy * qperpy * s002 + qperpz * qperpz * s002 -
             ...
             q0y * (q0x * (s012 - s112) + q0w * (s022 - s122)) +
             qperpw * qperpy * s122 + qperpx * qperpz * s122 +
             q0z * (-(q0w * s012) + q0x * s022 + q0w * s112 -
                    q0x * s122)) *
            theta
    );
    ...
    c2[1] = DerivativeTerm(
        ...
        qperpx * qperpy * s000 + qperpw * qperpz * s000 +
            ...
            q0x * (q0w * s020 + q0y * (-s000 + s100) -
                   q0w * s120 + 2 * qperpy * s000 * theta -
                   4 * qperpx * s010 * theta -
                   2 * qperpw * s020 * theta) +
            q0w * (-(q0z * s000) + q0z * s100 +
                   2 * qperpz * s000 * theta -
                   2 * qperpx * s020 * theta),
        qperpx * qperpy * s001 + qperpw * qperpz * s001 +
            ...
            q0x * (q0w * s021 + q0y * (-s001 + s101) -
                   q0w * s121 + 2 * qperpy * s001 * theta -
                   4 * qperpx * s011 * theta -
                   2 * qperpw * s021 * theta) +
            q0w * (-(q0z * s001) + q0z * s101 +
                   2 * qperpz * s001 * theta -
                   2 * qperpx * s021 * theta),
        qperpx * qperpy * s002 + qperpw * qperpz * s002 +
            ...
            q0x * (q0w * s022 + q0y * (-s002 + s102) -
                   q0w * s122 + 2 * qperpy * s002 * theta -
                   4 * qperpx * s012 * theta -
                   2 * qperpw * s022 * theta) +
            q0w * (-(q0z * s002) + q0z * s102 +
                   2 * qperpz * s002 * theta -
                   2 * qperpx * s022 * theta)
    );
    ...
    c2[2] = DerivativeTerm(
        0.,
        (q0w * q0y * s000 - q0x * q0z * s000 -
         qperpw * qperpy * s000 + qperpx * qperpz * s000 -
         q0w * q0x * s010 - q0y * q0z * s010 +
         qperpw * qperpx * s010 + qperpy * qperpz * s010 +
         q0x * q0x * s020 + q0y * q0y * s020 -
         qperpx * qperpx * s020 - qperpy * qperpy * s020 -
         q0w * q0y * s100 + q0x * q0z * s100 +
         qperpw * qperpy * s100 - qperpx * qperpz * s100 +
         q0w * q0x * s110 + q0y * q0z * s110 -
         qperpw * qperpx * s110 - qperpy * qperpz * s110 -
         q0x * q0x * s120 - q0y * q0y * s120 +
         qperpx * qperpx * s120 + qperpy * qperpy * s120 -
        ...
        (q0w * q0y * s001 - q0x * q0z * s001 -
         qperpw * qperpy * s001 + qperpx * qperpz * s001 -
         q0w * q0x * s011 - q0y * q0z * s011 +
         qperpw * qperpx * s011 + qperpy * qperpz * s011 +
         q0x * q0x * s021 + q0y * q0y * s021 -
         qperpx * qperpx * s021 - qperpy * qperpy * s021 -
         q0w * q0y * s101 + q0x * q0z * s101 +
         qperpw * qperpy * s101 - qperpx * qperpz * s101 +
         q0w * q0x * s111 + q0y * q0z * s111 -
         qperpw * qperpx * s111 - qperpy * qperpz * s111 -
         q0x * q0x * s121 - q0y * q0y * s121 +
         qperpx * qperpx * s121 + qperpy * qperpy * s121 -
        ...
        (q0w * q0y * s002 - q0x * q0z * s002 -
         qperpw * qperpy * s002 + qperpx * qperpz * s002 -
         q0w * q0x * s012 - q0y * q0z * s012 +
         qperpw * qperpx * s012 + qperpy * qperpz * s012 +
         q0x * q0x * s022 + q0y * q0y * s022 -
         qperpx * qperpx * s022 - qperpy * qperpy * s022 -
         q0w * q0y * s102 + q0x * q0z * s102 +
         qperpw * qperpy * s102 - qperpx * qperpz * s102 +
         q0w * q0x * s112 + q0y * q0z * s112 -
         qperpw * qperpx * s112 - qperpy * qperpz * s112 -
         q0x * q0x * s122 - q0y * q0y * s122 +
         qperpx * qperpx * s122 + qperpy * qperpy * s122 -
...
Ray AnimatedTransform::operator()(const Ray &r, Float *tMax)
    const
{
...
Ray AnimatedTransform::ApplyInverse(const Ray &r, Float *tMax)
    const
{
  if (!actuallyAnimated || r.time <= startTime)
...
RayDifferential AnimatedTransform::operator()(
...
Point3f AnimatedTransform::operator()(Point3f p, Float time)
    const
{
...
Vector3f AnimatedTransform::operator()(Vector3f v, Float time)
    const
{
  if (!actuallyAnimated || time <= startTime)
...
Normal3f AnimatedTransform::operator()(Normal3f n, Float time)
    const
{
  if (!actuallyAnimated || time <= startTime)
...
Interaction AnimatedTransform::operator()(const Interaction &it
) const
{
...
Interaction AnimatedTransform::ApplyInverse(
...
SurfaceInteraction AnimatedTransform::operator()(
...
SurfaceInteraction AnimatedTransform::ApplyInverse(
    const SurfaceInteraction &it
) const
{
  if (!actuallyAnimated) return startTransform.ApplyInverse(it);
...
Bounds3f AnimatedTransform::MotionBounds(const Bounds3f &b
) const
{
...
Bounds3f AnimatedTransform::BoundPointMotion(Point3f p) const
{
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    int nZeros = 0;
    FindZeros(
        c1[c].Eval(p), c2[c].Eval(p), c3[c].Eval(p),
    ...
    for (int i = 0; i < nZeros; ++i) {
      Point3f pz =
          (*this)(p, Lerp(zeros[i], startTime, endTime));
...
void AnimatedTransform::FindZeros(
    Float c1, Float c2, Float c3, Float c4, Float c5,
    Float theta, Interval tInterval, std::span<Float> zeros,
...
dadt.UpperBound() < 0 || dadt.LowerBound() == dadt.UpperBound())
  ...
  if (depth > 0 && dadt.Width() > 1e-3) {
    ...
    FindZeros(
        c1, c2, c3, c4, c5, theta,
        Interval(tInterval.LowerBound(), mid), zeros, nZeros,
        depth - 1
    );
    FindZeros(
        c1, c2, c3, c4, c5, theta,
        Interval(mid, tInterval.UpperBound()), zeros, nZeros,
        depth - 1
    );
  }
  else {
    ...
    for (int i = 0; i < 4; ++i) {
      // Evaluate motion function derivative and its derivative
      // at _tNewton_
      Float fNewton =
          c1 +
          (c2 + c3 * tNewton) * std::cos(2 * theta * tNewton) +
          (c4 + c5 * tNewton) * std::sin(2 * theta * tNewton);
      Float fPrimeNewton = (c3 + 2 * (c4 + c5 * tNewton) * theta
                           ) * std::cos(2 * tNewton * theta) +
                           (c5 - 2 * (c2 + c3 * tNewton) * theta
                           ) * std::sin(2 * tNewton * theta);
