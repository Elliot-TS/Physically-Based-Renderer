...
class Film {
 public:
  ...
  void AddSample(
      Vector3f color, int x, int y, unsigned int numSamples,
      unsigned int numNewSamples = 1
  )
  {
    ...
    int index = y * width + x;
    Vector3f col =
        imageSamples[index] * oldToTot + color * newToTot;
...
class Camera {
 public:
  ...
  Vector3f horizontal;
  Vector3f vertical;
  Film *film;  // I'm not understanding why I have to use a
               // pointer
  ...
  Camera(
      Film *film, Point3f origin, Vector3f direction,
      Float fov = 67, Float aspectRatio = 2
  )
      : origin(origin), film(film)
  {
    // Find the up vector
    Vector3f up(0, 1, 0);
    ...
    lower_left_corner =
        origin + direction - horizontal - vertical;
